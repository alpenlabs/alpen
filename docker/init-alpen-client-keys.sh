...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo "  OL_BLOCK_TIME_MS   OL block time in ms (default: 5000)"
            exit 0
            ;;
        -*)
...
if [ "${MODE}" = "sequencer" ]; then
    ...
    # Placeholder L1 view; strata/entrypoint.sh patches this at runtime with the actual L1 tip.
...
GEOF
    ...
    # --- sequencer.toml ---
    SEQUENCER_CONFIG="${OUTPUT_DIR}/sequencer.toml"
    if [ ! -f "${SEQUENCER_CONFIG}" ]; then
        cat > "${SEQUENCER_CONFIG}" <<SEOF
[sequencer]
...
slots_per_epoch = 64
SEOF
        echo "generated ${SEQUENCER_CONFIG}"
    fi
