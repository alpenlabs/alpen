...
fn get_program_dependencies() -> HashMap<&'static str, Vec<&'static str>> {
    ...
    dependencies.insert(OL_STF, vec![EVM_EE_STF]);
    dependencies.insert(CHECKPOINT, vec![OL_STF]);
    dependencies
}

fn main() {
    // List of guest programs to build
    let guest_programs = [EVM_EE_STF, OL_STF, CHECKPOINT];
