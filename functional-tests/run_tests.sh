...
build() {
    ...
    cargo build  -F sequencer -F debug-utils -F test-mode -F debug-asm --bin strata --bin strata-signer --bin alpen-client --bin strata-datatool --bin strata-test-cli --bin strata-dbtool --bin ee-da-verify
}
