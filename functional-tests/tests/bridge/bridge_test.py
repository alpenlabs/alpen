...
class BridgeTest(bridge_mixin.BridgeMixin):
    ...
    def main(self, ctx: flexitest.RunContext):
        ...
        drt_tx_id_1, deposit_id_1, dt_tx_id_1 = self.deposit(ctx, el_address, priv_keys)
        print(f"First deposit: DRT={drt_tx_id_1}, ID={deposit_id_1}, DT={dt_tx_id_1}")

        # Create second deposit using improved bridge manager
        drt_tx_id_2, deposit_id_2, dt_tx_id_2 = self.deposit(ctx, el_address, priv_keys)
        print(f"Second deposit: DRT={drt_tx_id_2}, ID={deposit_id_2}, DT={dt_tx_id_2}")

        # Verify deposits are tracked
        print(f"All stored deposits: {self.get_all_deposits()}")
        ...
        l2_tx_hash, tx_receipt, total_gas_used = self.withdraw(ctx, el_address, bosd)
        ...
        #
        # # Check final state
        remaining_intents = self.seqrpc.strata_getWithdrawalIntent()
