...
def write_reconstruction_config(
    ...
    *,
    ...
    """Write config for reconstruction-only runs without expected-root lookup."""
    lines = _base_verifier_config_lines(
        bitcoin,
        sequencer,
        magic_bytes_override=magic_bytes_override,
...
def write_verification_config(
    ...
    *,
    ...
    """Write config for runs that compare against the published expected root."""
    ...
    lines.extend(
        [
            f'ol_rpc_url = "{ol_rpc_url}"',
            f'ee_snark_account_id = "{ALPEN_ACCOUNT_ID}"',
        ]
    )
    return _write_config_file(bitcoin, lines, directory=directory)
...
def wait_for_expected_root_report(
    bitcoin: BitcoinService,
    sequencer: AlpenClientService,
    config_toml_path: str | Path,
    start_height: int,
    ...
    *,
    ...
    """Run the tool on a fixed L1 range until expected-root lookup catches up.
    ...
    for attempt in range(1, poll_attempts + 1):
        ...
        if "No canonical commitment found" in stderr:
            raise AssertionError(
                "alpen-ee-da-tool hit an unexpected expected-root RPC canonical-epoch error: "
                f"{stderr}"
            )
        if "incomplete expected-root comparison config" in stderr:
            raise AssertionError(
                "verification config is missing expected-root fields; "
                f"check write_verification_config(): {stderr}"
            )
        if "No Snark account update manifest found" not in stderr:
            raise AssertionError(
                ...
                f"expected-root lookup: {stderr}"
            )

        logger.info(
            "Attempt %s/%s: expected root not available yet over fixed range [%s, %s]: %s",
            attempt,
            poll_attempts,
            start_height,
            end_height,
    ...
    raise AssertionError(
        f"Expected-root lookup did not catch up for alpen-ee-da-tool; last_stderr={last_stderr}"
    )
