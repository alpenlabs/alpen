...
fn generate_test_tx(seed: u8) -> (OLTxId, OLTransaction) {
    ...
    let tx = OLTransaction::new(payload, attachment);
...
pub fn test_put_and_get_tx_entry(db: &impl MempoolDatabase) {
    let (txid, tx) = generate_test_tx(1);

    db.put_tx_entry(&txid, &tx).unwrap();

    let result = db.get_tx_entry(&txid).unwrap();
    assert_eq!(result, Some(tx));
}
...
pub fn test_update_existing_tx(db: &impl MempoolDatabase) {
    let (txid, tx1) = generate_test_tx(1);

    db.put_tx_entry(&txid, &tx1).unwrap();

    let (_, tx2) = generate_test_tx(2);
    // Update with a different blob
    db.put_tx_entry(&txid, &tx2).unwrap();

    let result = db.get_tx_entry(&txid).unwrap();
    assert_eq!(result, Some(tx2));
}

pub fn test_get_tx_entries_batch(db: &impl MempoolDatabase) {
    let (txid1, tx1) = generate_test_tx(1);
    let (txid2, tx2) = generate_test_tx(2);
    let (txid3, tx3) = generate_test_tx(3);

    db.put_tx_entry(&txid1, &tx1).unwrap();
    db.put_tx_entry(&txid2, &tx2).unwrap();
    db.put_tx_entry(&txid3, &tx3).unwrap();

    let txids = vec![txid1, txid2, txid3];
    let result = db.get_tx_entries(&txids).unwrap();

    assert_eq!(result.len(), 3);
    assert_eq!(result.get(&txid1), Some(&tx1));
    assert_eq!(result.get(&txid2), Some(&tx2));
    assert_eq!(result.get(&txid3), Some(&tx3));
}
