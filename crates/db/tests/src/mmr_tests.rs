...
pub fn test_append_single_leaf(db: &impl MmrDatabase) {
    let hash = [1u8; 32];

    // Initially empty
    assert_eq!(db.num_leaves().unwrap(), 0);
    assert_eq!(db.mmr_size().unwrap(), 0);

    // Append first leaf
    let idx = db.append_leaf(hash).unwrap();
    assert_eq!(idx, 0);
    assert_eq!(db.num_leaves().unwrap(), 1);
    assert_eq!(db.mmr_size().unwrap(), 1);
...
pub fn test_append_multiple_leaves(db: &impl MmrDatabase) {
    ...
    assert_eq!(db.num_leaves().unwrap(), 7);
    // 7 leaves with 3 peaks -> mmr_size = 2*7 - 3 = 11
    assert_eq!(db.mmr_size().unwrap(), 11);
}

pub fn test_get_node_positions(db: &impl MmrDatabase) {
    // Append 4 leaves
    ...
    assert_eq!(db.get_node(0).unwrap(), [0u8; 32]);
    assert_eq!(db.get_node(1).unwrap(), [1u8; 32]);
    assert_eq!(db.get_node(3).unwrap(), [2u8; 32]);
    assert_eq!(db.get_node(4).unwrap(), [3u8; 32]);

    // Internal nodes exist
    assert!(db.get_node(2).is_ok());
    assert!(db.get_node(5).is_ok());
    assert!(db.get_node(6).is_ok());
}

pub fn test_peak_roots(db: &impl MmrDatabase) {
    // Single leaf: one peak
    db.append_leaf([1u8; 32]).unwrap();
    let peaks = db.peak_roots();
    ...
    db.append_leaf([3u8; 32]).unwrap();
    let peaks = db.peak_roots();
    assert_eq!(peaks.len(), 2);

    // Four leaves: one peak (complete tree)
    db.append_leaf([4u8; 32]).unwrap();
    let peaks = db.peak_roots();
    assert_eq!(peaks.len(), 1);
}

pub fn test_pop_leaf_empty(db: &impl MmrDatabase) {
    // Popping from empty MMR returns None
    let result = db.pop_leaf().unwrap();
    assert_eq!(result, None);
}

pub fn test_pop_leaf_single(db: &impl MmrDatabase) {
    let hash = [42u8; 32];
    db.append_leaf(hash).unwrap();
...
pub fn test_pop_leaf_multiple(db: &impl MmrDatabase) {
    ...
    assert_eq!(db.num_leaves().unwrap(), 5);

    // Pop last leaf
    let popped = db.pop_leaf().unwrap();
    assert_eq!(popped, Some([4u8; 32]));
    assert_eq!(db.num_leaves().unwrap(), 4);

    // Pop another
    let popped = db.pop_leaf().unwrap();
    assert_eq!(popped, Some([3u8; 32]));
    assert_eq!(db.num_leaves().unwrap(), 3);
}

pub fn test_append_after_pop(db: &impl MmrDatabase) {
    // Append 3 leaves
    db.append_leaf([1u8; 32]).unwrap();
    db.append_leaf([2u8; 32]).unwrap();
    db.append_leaf([3u8; 32]).unwrap();
    ...
    // Pop one
    db.pop_leaf().unwrap();
    assert_eq!(db.num_leaves().unwrap(), 2);

    // Append again
    let idx = db.append_leaf([4u8; 32]).unwrap();
    assert_eq!(idx, 2);
    assert_eq!(db.num_leaves().unwrap(), 3);
}

pub fn test_to_compact(db: &impl MmrDatabase) {
    // Empty MMR
...
pub fn test_mmr_size_formula(db: &impl MmrDatabase) {
