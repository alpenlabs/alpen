...
pub fn proptest_put_and_get_preseal_ol_state(
    db: &impl OLStateDatabase,
...
pub fn proptest_delete_preseal_ol_state(
    db: &impl OLStateDatabase,
    commitment: EpochCommitment,
    state: OLState,
) {
    db.put_preseal_ol_state(commitment, state)
        .expect("test: put preseal");
...
macro_rules! ol_state_db_tests {
    ($setup_expr:expr) => {
        proptest::proptest! {
            ...
            #[test]
            fn proptest_put_and_get_preseal_ol_state(
                commitment in strata_identifiers::test_utils::epoch_commitment_strategy(),
                state in strata_ol_state_types::test_utils::ol_state_strategy(),
            ) {
                let db = $setup_expr;
                $crate::ol_state_tests::proptest_put_and_get_preseal_ol_state(&db, commitment, state);
            }

            #[test]
            fn proptest_delete_preseal_ol_state(
                commitment in strata_identifiers::test_utils::epoch_commitment_strategy(),
                state in strata_ol_state_types::test_utils::ol_state_strategy(),
            ) {
                let db = $setup_expr;
                $crate::ol_state_tests::proptest_delete_preseal_ol_state(&db, commitment, state);
            }
