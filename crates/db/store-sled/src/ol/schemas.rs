...
use crate::{
    ...
    impl_borsh_key_codec, impl_borsh_value_codec,
};

define_table_without_codec!(
    /// A table to store OL Block data. Maps block commitment to Block
    (OLBlockSchema) OLBlockCommitment => OLBlock
);

impl_borsh_key_codec!(OLBlockSchema, OLBlockCommitment);
