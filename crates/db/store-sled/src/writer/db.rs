...
impl L1WriterDatabase for L1WriterDBSled {
    ...
    fn requeue_abandoned_intent(
        ...
        self.config
            .with_retry((&self.payload_tree, &self.intent_tree), |(pt, it)| {
                let Some(intent) = it.get(&intent_id)? else {
                    return Ok(None);
                };
                if intent.status != IntentStatus::Bundled(abandoned_payload_idx)
                    || pt
                        .get(&abandoned_payload_idx)?
                ...
                let payload_idx = find_next_available_id(&pt, next_payload_idx)?;
                pt.insert(&payload_idx, &payloadentry)?;
                let mut updated_intent = intent;
                updated_intent.status = IntentStatus::Bundled(payload_idx);
                it.insert(&intent_id, &updated_intent)?;
                Ok(Some(payload_idx))
            })
    }
