...
///
/// This macro is identical to `impl_ssz_transparent_buf32_wrapper!` - the `_copy` suffix is
/// kept for backwards compatibility. `ToOwnedSsz` is now provided via a blanket impl for
/// all `Copy` types in `ssz_types::view`, so no explicit impl is needed.
///
/// # Example
///
/// ```ignore
