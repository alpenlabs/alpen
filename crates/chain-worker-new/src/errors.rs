...
pub enum WorkerError {
    ...
    /// An OL log named a snark account that is missing or of the wrong type in
    /// the reconstructed post-state. This is a state-divergence condition: the
    /// log only exists because the account produced an update during the epoch,
    /// so its post-state must be a snark account.
    #[error("post-state has no snark account for log target {0}")]
    MissingSnarkAccountForLog(AccountId),
