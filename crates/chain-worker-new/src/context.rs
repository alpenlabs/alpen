...
pub struct ChainWorkerContextImpl {
    ...
    epoch_summary_tx: broadcast::Sender<EpochCommitment>,
...
impl ChainWorkerContextImpl {
    ...
    pub fn from_node_context(nodectx: &NodeContext) -> Self {
        let (epoch_summary_tx, _) = broadcast::channel(64);
        Self {
    ...
    pub fn epoch_summary_receiver(&self) -> broadcast::Receiver<EpochCommitment> {
        self.epoch_summary_tx.subscribe()
    }

    pub fn epoch_summary_sender(&self) -> broadcast::Sender<EpochCommitment> {
...
impl ChainWorkerContext for ChainWorkerContextImpl {
    ...
    fn store_summary(&self, summary: EpochSummary) -> WorkerResult<()> {
        ...
        let _ = self.epoch_summary_tx.send(commitment);
        Ok(())
    }
