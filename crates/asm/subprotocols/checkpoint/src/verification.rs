...
use strata_identifiers::{
    Buf32, CredRule, Epoch, L1BlockCommitment, L2BlockCommitment, Slot, hash::raw,
};
...
/// Validate that the payload's L1 range start matches the expected value from state.
///
/// The L1 range start in the payload must exactly match the last checkpoint L1 block
/// from state to ensure continuity and prevent gaps/overlaps.
pub(crate) fn validate_l1_range_start(
    state: &CheckpointState,
    payload_l1_start: &L1BlockCommitment,
) -> CheckpointResult<()> {
    let expected = state.last_checkpoint_l1();
    if payload_l1_start != expected {
        return Err(crate::error::CheckpointError::InvalidL1RangeStart {
            expected: *expected,
            actual: *payload_l1_start,
        });
    }
    Ok(())
}

/// Validate that the payload's L2 range start matches the expected value from state.
///
/// The L2 range start in the payload must exactly match the last L2 terminal block
/// from state (or null for the first checkpoint) to ensure continuity.
pub(crate) fn validate_l2_range_start(
    state: &CheckpointState,
    payload_l2_start: &L2BlockCommitment,
) -> CheckpointResult<()> {
    let expected = state
        .last_l2_terminal()
        .copied()
        .unwrap_or_else(L2BlockCommitment::null);
    if payload_l2_start != &expected {
        return Err(crate::error::CheckpointError::InvalidL2RangeStart {
            expected,
            actual: *payload_l2_start,
        });
    }
    Ok(())
}

/// Validate that the payload's pre-state root matches the expected value from state.
///
/// The pre-state root in the payload must exactly match the final state from the
/// last verified epoch (or zero for the first checkpoint) to ensure state continuity.
pub(crate) fn validate_pre_state_root(
    state: &CheckpointState,
    payload_pre_state: &Buf32,
) -> CheckpointResult<()> {
    let expected = state
        .verified_epoch_summary()
        .map(|s| *s.final_state())
        .unwrap_or_else(Buf32::zero);
    if payload_pre_state != &expected {
        return Err(crate::error::CheckpointError::InvalidPreStateRoot {
            expected,
