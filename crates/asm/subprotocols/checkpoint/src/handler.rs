...
use crate::{
    ...
    verification::{
        ...
        validate_l1_range_start, validate_l2_progression, validate_l2_range_start,
        validate_pre_state_root, verify_checkpoint_proof, verify_checkpoint_signature,
    },
};
...
pub(crate) fn handle_checkpoint_tx(
    ...
    // 3. Validate state transitions (end values must progress)
    ...
    // 4. Validate start values match expected state (defense-in-depth)
    // These ensure the payload's claimed start points match our state, preventing
    // gaps/overlaps and ensuring logged data accurately reflects verified transitions.
    validate_l1_range_start(state, batch_info.l1_range().start())?;
    validate_l2_range_start(state, batch_info.l2_range().start())?;
    validate_pre_state_root(state, checkpoint.transition().pre_state_root())?;

    // 5. Get manifest hashes from auxiliary data (requested during pre-process phase)
    ...
    // 6. Construct claim and verify proof (start values from state, end values from payload)
