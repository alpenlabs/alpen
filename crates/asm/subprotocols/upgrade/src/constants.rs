/// The number of blocks during which an ASM verification key (VK) update remains in the
/// `QueuedUpgrade` state and can still be cancelled by submitting a cancel transaction.
/// If no cancellation occurs within this many blocks of the original update transaction,
/// the upgrade transitions to `CommittedUpgrade` and can no longer be cancelled.
pub const ASM_VK_QUEUE_DELAY: u64 = 12_960;

/// The number of blocks during which an optimistic L1 state-transition function (OL STF)
/// verification key (VK) update remains in the `QueuedUpgrade` state and can still be
/// cancelled by submitting a cancel transaction. If no cancellation occurs within this
/// many blocks of the original update transaction, the upgrade transitions to
/// `CommittedUpgrade` and can no longer be cancelled.
pub const OL_STF_VK_QUEUE_DELAY: u64 = 4_320;

// ─── Enactment Delays ───────────────────────────────────────────────────────────

/// The number of blocks between when a multisig configuration update enters the
/// `ScheduledUpgrade` state and when it is actually enacted. During this period, the
/// update is immutable and cannot be cancelled.
pub const MULTISIG_CONFIG_UPDATE_ENACTMENT_DELAY: u64 = 2_016;

/// The number of blocks between when an operator update enters the `ScheduledUpgrade`
/// state and when it is actually enacted. During this period, the update is immutable
/// and cannot be cancelled.
pub const OPERATOR_UPDATE_ENACTMENT_DELAY: u64 = 2_016;

/// The number of blocks between when a sequencer update enters the `ScheduledUpgrade`
/// state and when it is actually enacted. During this period, the update is immutable
/// and cannot be cancelled.
pub const SEQUENCER_UPDATE_ENACTMENT_DELAY: u64 = 2_016;

/// The number of blocks between when a verification key (VK) update enters the
/// `ScheduledUpgrade` state and when it is actually enacted. During this period, the
/// update is immutable and cannot be cancelled.
pub const VK_UPDATE_ENACTMENT_DELAY: u64 = 144;
