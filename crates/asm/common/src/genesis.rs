...
//!
//! This module provides a registry for managing genesis configurations
//! that are used to initialize subprotocol states during genesis block
//! processing or when new subprotocols are added.
...
/// Registry for managing genesis configurations for all subprotocols.
///
/// This registry stores serialized genesis configurations that are used
/// when initializing subprotocol states. The configurations are keyed
/// by subprotocol ID and stored in serialized form to avoid type dependencies.
#[derive(Debug, Clone, BorshSerialize, BorshDeserialize)]
pub struct GenesisConfigRegistry {
    /// Map of subprotocol ID to serialized genesis configuration
    configs: BTreeMap<SubprotocolId, Vec<u8>>,
}

impl GenesisConfigRegistry {
    /// Creates a new empty genesis configuration registry.
    pub fn new() -> Self {
        Self {
    ...
    pub fn register<T: BorshSerialize>(
        ...
        let serialized = borsh::to_vec(config)
            .map_err(|e| AsmError::SerializationError(e.to_string()))?;
...
impl Default for GenesisConfigRegistry {
    fn default() -> Self {
        Self::new()
    }
}
