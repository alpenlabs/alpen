...
/// 
/// This trait focuses on the core cryptographic operations: key aggregation and signature verification.
/// Higher-level multisig workflow is handled by generic functions that use this trait.
pub trait CryptoScheme: Clone + Debug + Send + Sync + 'static {
    /// The type used to represent a public key in this scheme.
    type PubKey: Clone + Debug + PartialEq + Eq + Send + Sync + BorshSerialize + BorshDeserialize + for<'a> Arbitrary<'a>;
    
    /// The type used to represent a signature in this scheme.
    type Signature: Clone + Debug + PartialEq + Eq + Send + Sync + Default;
