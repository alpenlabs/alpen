...
use crate::{
    state::{self, OLSyncState},
    OLSyncError, SyncClient,
};
...
) -> Result<EpochCommitment, OLSyncError> {
    let finalized_epoch = chainstatus_rx
        .wait_for(wait_for_fn)
        .await
        .map_err(|_| OLSyncError::ChannelClosed)?
        .as_ref()
...
async fn handle_finalized_epoch<T: SyncClient>(
    ...
    state: &mut OLSyncState,
    _context: &OLSyncContext<T>,
) {
...
async fn handle_new_block<T: SyncClient>(
    ...
    loop {
        ...
        if height <= state.finalized_height() {
            ...
            return Err(OLSyncError::WrongFork(block_id, height));
        }

        let parent_block_id = block.header().parent_blkid();
        ...
        let Some(parent_block) = context.client.get_block_by_id(parent_block_id).await? else {
            ...
            return Err(OLSyncError::MissingBlock(*parent_block_id));
        };
    ...
    while let Some(block) = fetched_blocks.pop() {
        state.attach_block(block.signed_header())?;
        // TODO: Update this to use OL storage when available
        context
            .storage
            .ol_block()
...
async fn handle_block_finalized(
    ...
    if !state.has_block(new_finalized_epoch.last_blkid()) {
        return Err(OLSyncError::MissingFinalized(
