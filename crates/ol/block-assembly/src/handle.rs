...
impl BlockasmHandle {
    ...
    /// Look up or generate a pending block template by parent block ID.
    pub async fn get_or_generate_block_template(
        &self,
        parent_block_id: OLBlockId,
    ) -> BlockAssemblyResult<FullBlockTemplate> {
        let (completion, rx) = create_completion();
        let command = BlockasmCommand::GetOrGenerateBlockTemplate {
            parent_block_id,
            completion,
        };
        self.send_command(command, rx).await?
