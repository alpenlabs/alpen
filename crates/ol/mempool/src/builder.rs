...
impl MempoolBuilder {
    ...
    pub async fn launch(self, texec: &TaskExecutor) -> anyhow::Result<MempoolHandle> {
        ...
        let (command_tx, command_rx) = tokio::sync::mpsc::channel(self.config.command_buffer_size);
...
impl MempoolInput {
    fn new(
        command_rx: tokio::sync::mpsc::Receiver<MempoolCommand>,
        chain_sync_rx: tokio::sync::watch::Receiver<Option<ChainSyncStatusUpdate>>,
    ) -> Self {
        Self {
            command_rx,
            chain_sync_rx,
