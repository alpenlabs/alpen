...
/// but stores only the [`base_update`](SnarkAccountUpdate) without the `accumulator_proofs` that
/// are
...
///
/// Provides deterministic FIFO ordering across restarts.
/// Ordered by (`first_seen_slot`, `insertion_id`) to ensure:
/// - Earlier slots have higher priority
/// - Within same slot, earlier insertions have higher priority
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct MempoolOrderingKey {
    /// Slot when transaction was first seen.
    pub first_seen_slot: u64,
    /// Monotonic counter for insertion order within same slot.
    pub insertion_id: u64,
}

impl MempoolOrderingKey {
    /// Create a new ordering key.
    pub fn new(first_seen_slot: u64, insertion_id: u64) -> Self {
        Self {
            first_seen_slot,
            insertion_id,
        }
...
pub(crate) struct MempoolEntry {
    ...
    /// Ordering key for FIFO ordering (slot + insertion order).
...
pub enum OLMempoolRejectReason {
    /// Rejected due to mempool full (capacity limit reached).
    MempoolFull,
    ...
    /// Rejected due to not yet valid (min_slot in future).
    NotYetValid,
...
impl OLMempoolRejectReason {
    /// Try to extract a rejection reason from an [`OLMempoolError`](crate::OLMempoolError).
    ///
