...
use strata_ol_tx_types_v1::{
    ...
    TransactionPayloadV1, TxConstraintsV1, TxProofsV1, test_utils as ol_test_utils,
};
...
pub(crate) fn create_test_constraints() -> TxConstraintsV1 {
    let mut runner = TestRunner::default();
...
pub(crate) fn create_test_snark_update() -> SnarkAccountUpdate {
    // Use ol-chain-types strategy to generate a SauTxPayloadV1, then extract a SnarkAccountUpdate
    let mut runner = TestRunner::default();
...
pub(crate) fn create_test_constraints_with_slots(
    ...
    TxConstraintsV1::new(min_slot, max_slot)
}
...
pub(crate) fn create_test_snark_tx_from_update(
    ...
    let sau_proof_state =
        SauTxProofStateV1::new(proof_state.next_inbox_msg_idx(), proof_state.inner_state());
...
pub(crate) fn tx_target(tx: &OLTransactionV1) -> AccountId {
    tx.target()
        .expect("all OLTransactionV1 payload variants must have a target account")
}

/// Returns the snark sequence number for snark transactions.
///
/// Returns `None` for non-snark payload variants.
pub(crate) fn snark_seq_no(tx: &OLTransactionV1) -> Option<u64> {
    match tx.payload() {
...
pub(crate) fn with_constraints(
    ...
    OLTransactionV1::new(data, tx.proofs().clone())
}

/// Returns a transaction with updated `max_slot` in constraints.
pub(crate) fn with_max_slot(tx: OLTransactionV1, max_slot: Option<Slot>) -> OLTransactionV1 {
...
/// Create a test OLStateV1 with a Snark account for testing SnarkAccountUpdate transactions.
///
/// # Arguments
...
///
/// # Returns
/// An `OLStateV1` with the specified Snark account at the specified slot
...
pub(crate) fn create_test_snark_tx() -> OLTransactionV1 {
...
pub(crate) fn create_test_generic_tx() -> OLTransactionV1 {
...
pub(crate) fn create_test_generic_tx_with_constraints(
    ...
    let data = OLTransactionDataV1::from_gam_bytes(target, payload)
        .expect("message payload bytes must fit within SSZ max length")
        .with_constraints(constraints);
    OLTransactionV1::new(data, TxProofsV1::new_empty())
}
...
pub(crate) fn create_test_tx_with_id(id: u8) -> OLTransactionV1 {
...
pub(crate) fn create_test_snark_tx_with_seq_no(account_id: u8, seq_no: u64) -> OLTransactionV1 {
...
pub(crate) fn create_test_generic_tx_for_account(account_id: u8) -> OLTransactionV1 {
    let constraints = create_test_constraints_with_slots(None, None);
