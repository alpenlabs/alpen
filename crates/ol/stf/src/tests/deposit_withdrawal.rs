...
fn test_snark_account_deposit_and_withdrawal() {
    let snark_account_id = make_account_id(TEST_SNARK_ACCOUNT_ID);
    ...
    let snark_serial = genesis.next_account_serial();
    let genesis_manifest =
        make_deposit_manifest_for_account(1, 1, snark_serial, dest_subject, deposit_amount);

    let mut fixture = genesis
        .with_genesis_snark_account(snark_account_id, |acct| {
    ...
    assert_eq!(
        fixture.account_balance(snark_account_id),
        deposit_amount,
        "Account balance should reflect the deposit"
    );

    let snark_state_after_genesis = fixture.expect_snark_account(snark_account_id);
    assert_eq!(
        snark_state_after_genesis.next_inbox_msg_idx(),
        0,
        "Next inbox idx should still be zero (no messages processed yet)"
    );
    assert_eq!(
        snark_state_after_genesis.inbox_mmr().num_entries(),
        1,
