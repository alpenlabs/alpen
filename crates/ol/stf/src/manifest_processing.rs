...
fn process_asm_log<S: IStateAccessor>(
    ...
    match msg.ty() {
        DEPOSIT_LOG_TYPE_ID => {
            // Parse the deposit log, skip if it fails to parse.
            let Ok(data) = log.try_into_log::<DepositLog>() else {
                return Ok(());
            };
            process_deposit_log(state, &data, context)?;
        }
...
fn process_deposit_log<S: IStateAccessor>(
    ...
    // Decode the deposit descriptor from the raw destination bytes.
    let descriptor = DepositDescriptor::decode_from_slice(&data.destination)
        .map_err(|_| ExecError::ChainIntegrity)?;

    let dest_acct_serial = *descriptor.dest_acct_serial();
    let dest_subject = descriptor.dest_subject().to_subject_id();

    // Convert the account serial to account ID.
    let Some(dest_id) = state.find_account_id_by_serial(dest_acct_serial)? else {
    ...
    let msg_payload = MsgPayload::new(data.amount.into(), msg_data);
