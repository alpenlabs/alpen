//! # Genesis logic for the Orchestration Layer (OL) in the Strata node.
//!
//! This module handles initialization of the OL genesis state and block for new Strata databases.
//! It sets up the initial OL state, block info, and mimics rollup parameters as provided by the
//! configured genesis L1 view. The implementation is designed to work with Alpen and anticipates
//! correct manifest bootstrapping from the ASM.
...
pub fn init_ol_genesis(params: &Params, storage: &NodeStorage) -> Result<OLBlockCommitment> {
