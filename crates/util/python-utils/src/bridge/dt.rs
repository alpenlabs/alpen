...
fn create_deposit_transaction_inner(
    ...
    let (mut psbt, prevouts, tweak) = build_deposit_tx(&deposit_request_data, agg_pubkey)?;

    // Sign with MuSig2
    let signer = MusigSigner::new();
    let signature = signer.sign_deposit_psbt(&psbt, &prevouts, tweak, operator_secret_keys, 0)?;
...
fn create_metadata_script_direct(metadata: &DepositTxMetadata) -> Result<ScriptBuf, Error> {
    ...
    buf.extend_from_slice(&metadata.ee_address);
    buf.extend_from_slice(metadata.takeback_hash.as_ref());
    buf.extend_from_slice(&metadata.input_amount.to_sat().to_be_bytes());

    let push_data = PushBytesBuf::try_from(buf)
