...
use bdk_wallet::{
    bitcoin::{
        consensus::{deserialize, serialize}, key::UntweakedPublicKey, 
        locktime::absolute::LockTime, opcodes::all::OP_RETURN, script::{Builder, PushBytesBuf}, taproot::{LeafVersion, TaprootBuilder, TaprootSpendInfo}, transaction::Version, Address, Amount, Network, OutPoint, Psbt, ScriptBuf, Sequence, TapNodeHash, TapSighashType, Transaction, TxIn, TxOut, TxOut as BitcoinTxOut, Witness, XOnlyPublicKey
    },
...
pub(crate) fn create_deposit_transaction(
    ...
    // Validate operator keys -> Vec<[u8; 78]>
    let mut fixed_keys = Vec::with_capacity(operator_keys.len());
    for key in operator_keys {
        if key.len() != 78 {
            return Err(PyValueError::new_err(format!(
                "expected operator key of 78 bytes, got {}",
                key.len()
            )));
        }
        let mut arr = [0u8; 78];
    ...
    let signers = parse_operator_keys(&fixed_keys)?;
...
mod tests {
    ...
    use super::*;
    use crate::constants::{BRIDGE_OUT_AMOUNT, GENERAL_WALLET_KEY_PATH, MAGIC_BYTES, XPRIV};
    ...
    fn test_build_deposit_tx_structure() {
        ...
        let drt_data = DepositRequestInfo {
            ...
            take_back_leaf_hash: [0u8; 32], // Mock takeback hash
        };
