...
mod tests {
    ...
    use strata_checkpoint_types_ssz::{test_utils::checkpoint_payload_strategy, CheckpointPayload};
    ...
    proptest! {
        #[test]
        fn proptest_insert_and_get_epoch_summary_blocking(bytes in any::<Vec<u8>>()) {
            let mut input = Unstructured::new(&bytes);
            let summary: EpochSummary = match Arbitrary::arbitrary(&mut input) {
                Ok(summary) => summary,
                Err(_) => return Ok(()),
            };
    ...
    proptest! {
        #[test]
        fn proptest_insert_and_get_epoch_summary_async(bytes in any::<Vec<u8>>()) {
            let mut input = Unstructured::new(&bytes);
            let summary: EpochSummary = match Arbitrary::arbitrary(&mut input) {
                Ok(summary) => summary,
                Err(_) => return Ok(()),
        ...
        #[test]
        fn proptest_del_epoch_summary_async(bytes in any::<Vec<u8>>()) {
            let mut input = Unstructured::new(&bytes);
            let summary: EpochSummary = match Arbitrary::arbitrary(&mut input) {
                Ok(summary) => summary,
                Err(_) => return Ok(()),
    ...
    #[tokio::test]
    async fn test_get_last_checkpoint_epoch_async() {
        let manager = setup_manager();
        ...
        // Generate test data using Arbitrary
        let bytes: Vec<u8> = (0..512).map(|i| i as u8).collect();
        let mut input = Unstructured::new(&bytes);
        let payload: CheckpointPayload = Arbitrary::arbitrary(&mut input).unwrap();
        let entry = OLCheckpointEntry::new_unsigned(payload);
    ...
    #[tokio::test]
    async fn test_status_transition_async() {
        let manager = setup_manager();
