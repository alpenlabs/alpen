...
impl MempoolManager {
    ...
    pub fn put_tx_entry(&self, txid: &OLTxId, tx: &OLTransaction) -> DbResult<()> {
        self.db.put_tx_entry(txid, tx)
    }

    /// Get a transaction from the mempool
    pub fn get_tx_entry(&self, txid: &OLTxId) -> DbResult<Option<OLTransaction>> {
        self.db.get_tx_entry(txid)
    }

    /// Get multiple transactions from the mempool
    pub fn get_tx_entries(&self, txids: &[OLTxId]) -> DbResult<HashMap<OLTxId, OLTransaction>> {
...
mod tests {
    ...
    fn create_test_tx(seed: u8) -> (OLTxId, OLTransaction) {
    ...
    fn test_mempool_manager_basic_operations() {
        ...
        let (txid1, tx1) = create_test_tx(1);
        let (txid2, tx2) = create_test_tx(2);

        // Test put and get
        manager.put_tx_entry(&txid1, &tx1).unwrap();
        let result = manager.get_tx_entry(&txid1).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap(), tx1);

        // Test batch get
        manager.put_tx_entry(&txid2, &tx2).unwrap();
        let batch_result = manager.get_tx_entries(&[txid1, txid2]).unwrap();
        assert_eq!(batch_result.len(), 2);
        assert_eq!(batch_result.get(&txid1), Some(&tx1));
        assert_eq!(batch_result.get(&txid2), Some(&tx2));
