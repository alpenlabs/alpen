...
use anyhow::Context;
pub use managers::{
    asm::AsmManager, chainstate::ChainstateManager, checkpoint::CheckpointDbManager,
...
impl NodeStorage {
    pub fn asm(&self) -> &Arc<AsmManager> {
        &self.asm_manager
    }
    ...
    D: DatabaseBackend + 'static,
{
    ...
    let asm_manager = Arc::new(AsmManager::new(pool.clone(), asm_db));
