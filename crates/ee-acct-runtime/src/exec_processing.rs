...
use crate::verification_state::{InputTracker, PendingCommit, UpdateTransitionVerificationState};
...
struct ChainVerificationState<'v, 'a, E: ExecutionEnvironment> {
    utvstate: &'v mut UpdateTransitionVerificationState,
...
impl<'v, 'a, E: ExecutionEnvironment> ChainVerificationState<'v, 'a, E> {
    ...
    fn next_pending_commit(&self) -> Option<&PendingCommit> {
        self.utvstate.pending_commits().get(self.processed_commits)
    }
    ...
    fn consume_pending_inputs_from_block(&mut self, block_inputs: &BlockInputs) -> EnvResult<()> {
        ...
        self.utvstate.inc_consumed_inputs(input_count);
        Ok(())
    }
...
pub(crate) fn process_segments<E: ExecutionEnvironment>(
    utvstate: &mut UpdateTransitionVerificationState,
    ...
    let mut cvstate =
        ChainVerificationState::new(utvstate, input_tracker, ee, partial_pre_state, header);
