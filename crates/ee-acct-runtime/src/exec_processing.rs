...
struct ChainVerificationState<'v, 'a, 'b, E: ExecutionEnvironment> {
    vstate: &'v mut EeVerificationState<'a, E>,
    input_tracker: &'v mut SequenceTracker<'b, PendingInputEntry>,
...
impl<'v, 'a, 'b, E: ExecutionEnvironment> ChainVerificationState<'v, 'a, 'b, E> {
    fn new(
        vstate: &'v mut EeVerificationState<'a, E>,
        input_tracker: &'v mut SequenceTracker<'b, PendingInputEntry>,
        exec_state: E::PartialState,
    ...
    fn next_pending_commit(&self) -> Option<&PendingCommit> {
        self.vstate.pending_commits().get(self.processed_commits)
    }
    ...
    fn consume_pending_inputs_from_block(&mut self, block_inputs: &ExecInputs) -> EnvResult<()> {
        ...
        self.vstate.inc_consumed_inputs(input_count);
        Ok(())
    }
    ...
    fn apply_write_batch(
        ...
        self.vstate
            .ee()
            .merge_write_into_state(&mut self.exec_state, wb)?;
        let new_sr = self.compute_cur_state_root()?;

        if new_sr != h.get_state_root() {
            // maybe mismatched terminal state sometimes?
            return Err(EnvError::MismatchedIntermediateState);
        }
...
///
/// This function takes the verification state (which contains the commit data,
/// pre-state, header, and execution environment) and processes all chain
/// segments to verify block execution.
pub(crate) fn process_segments<'a, 'b, E: ExecutionEnvironment>(
    vstate: &mut EeVerificationState<'a, E>,
    input_tracker: &mut SequenceTracker<'b, PendingInputEntry>,
) -> EnvResult<()> {
    // Get required data from vstate
    let segments = vstate.commit_data();
    let pre_state = vstate.raw_partial_pre_state();
    let cur_tip_header = vstate.raw_prev_header();
    ...
    let mut cvstate = ChainVerificationState::new(vstate, input_tracker, partial_pre_state, header);
...
fn process_chain_segment<E: ExecutionEnvironment>(
    cvstate: &mut ChainVerificationState<'_, '_, '_, E>,
