...
pub enum AlpenGossipCommand {
    ...
    SendMessage(AlpenGossipMessage),
}
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            ...
            if let Poll::Ready(Some(cmd)) = this.commands.poll_next_unpin(cx) {
                return match cmd {
                    AlpenGossipCommand::SendMessage(message) => Poll::Ready(Some(message.encode())),
                };
            }
            ...
            let msg = match AlpenGossipMessage::try_decode(&mut &msg[..]) {
                Ok(msg) => msg,
                Err(e) => {
                    // TODO(@storopoli): maybe disconnect on invalid message or invalid sequencer's
                    //                   signature.
                    error!(?e, "failed to decode gossip message");
                    return Poll::Ready(None);
                }
            };

            this.events
                .send(AlpenGossipEvent::Message {
