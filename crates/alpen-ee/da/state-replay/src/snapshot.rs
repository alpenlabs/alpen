...
/// state seed used to initialize the reconstructor plus bytecode preimages for
/// accounts that existed before the replayed DA range.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReplayPreStateSnapshot {
    expected_state_root: Buf32,
    next_update_seq_no: u64,
    last_applied_block_num: u64,
    state_seed: StateReconstructorSeed,
...
impl ReplayPreStateSnapshot {
    ...
    /// Returns the expected root of the seeded pre-state.
    ...
    /// Returns the state seed used to initialize the reconstructor.
    pub fn state_seed(&self) -> &StateReconstructorSeed {
        &self.state_seed
    }
...
mod tests {
    ...
    fn replay_pre_state_snapshot_json_roundtrips() {
        ...
        let state_seed = StateReconstructorSeed::new(
