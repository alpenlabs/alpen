...
use strata_db_store_sled::{
    ...
    impl_codec_value_codec, impl_opaque_value_codec, impl_raw_bytes_key_codec,
};
...
define_table_without_codec!(
    ...
    (OLBlockAtEpochSchema) u32 => OLBlockId
);
// `u32` keys use typed-sled's blanket big-endian integer key codec.
impl_codec_value_codec!(OLBlockAtEpochSchema, OLBlockId);

define_table_without_codec!(
    /// EeAccountState at specific OL Block
    (AccountStateAtOLEpochSchema) OLBlockId => DBAccountStateAtEpoch
);
impl_codec_key_codec!(AccountStateAtOLEpochSchema, OLBlockId);
impl_cbor_value_codec!(AccountStateAtOLEpochSchema, DBAccountStateAtEpoch);

define_table_without_codec!(
    /// ExecBlock by blockhash
    (ExecBlockSchema) Hash => DBExecBlockRecord
);
impl_codec_key_codec!(ExecBlockSchema, Hash);
impl_cbor_value_codec!(ExecBlockSchema, DBExecBlockRecord);
...
impl_codec_value_codec!(ExecBlockFinalizedSchema, Hash);

define_table_without_codec!(
    /// ExecBlock payloads
    (ExecBlockPayloadSchema) Hash => Vec<u8>
);
impl_codec_key_codec!(ExecBlockPayloadSchema, Hash);
impl_opaque_value_codec!(ExecBlockPayloadSchema, Vec<u8>);
...
impl_cbor_value_codec!(BatchByIdxSchema, DBBatchWithStatus);

define_table_without_codec!(
    /// BatchId -> idx lookup
    (BatchIdToIdxSchema) DBBatchId => u64
);
impl_codec_value_codec!(BatchIdToIdxSchema, u64);

define_table_without_codec!(
    /// Chunk by sequential idx -> (Chunk, Status)
    (ChunkByIdxSchema) u64 => DBChunkWithStatus
);
impl_cbor_value_codec!(ChunkByIdxSchema, DBChunkWithStatus);

define_table_without_codec!(
    /// ChunkId -> idx lookup
    (ChunkIdToIdxSchema) DBChunkId => u64
);
impl_codec_value_codec!(ChunkIdToIdxSchema, u64);

define_table_without_codec!(
    /// Batch-Chunk association
    (BatchChunksSchema) DBBatchId => Vec<DBChunkId>
);
impl_cbor_value_codec!(BatchChunksSchema, Vec<DBChunkId>);
...
impl_codec_key_codec!(BlockAccessedStateSchema, Hash);
impl_cbor_value_codec!(BlockAccessedStateSchema, AccessedStateRecord);
...
impl_codec_key_codec!(BytecodeSchema, Hash);
impl_opaque_value_codec!(BytecodeSchema, Vec<u8>);
...
impl_opaque_value_codec!(EeBlockWitnessSchema, Vec<u8>);
...
define_table_without_codec!(
    ...
    /// the caller side (`ChunkTask` / `BatchTask`). Stored verbatim so the kind tag stays
    /// the first byte and prefix seeks work.
...
define_table_without_codec!(
    ...
    /// shape matches paas's `ReceiptStore`. Receipts are opaque to the database.
    (ChunkProofReceiptSchema) Vec<u8> => ProofReceiptEntry
);
impl_raw_bytes_key_codec!(ChunkProofReceiptSchema, Vec<u8>);
impl_opaque_value_codec!(ChunkProofReceiptSchema, ProofReceiptEntry);

define_table_without_codec!(
    /// Acct (outer/update) proof receipts keyed by [`BatchId`].
    ///
    /// Receipts are opaque to the database.
    (AcctProofReceiptSchema) DBBatchId => ProofReceiptEntry
);
impl_opaque_value_codec!(AcctProofReceiptSchema, ProofReceiptEntry);
...
impl_codec_key_codec!(AcctProofIdIndexSchema, Hash);
impl_cbor_value_codec!(AcctProofIdIndexSchema, DBBatchId);
