...
use crate::{AlpenForkSchedule, BlobSpec, EvmSpec};
...
/// account identity, bridge economics, DA stream identity, the Alpen fork
/// schedule, and the embedded EVM chain spec. Loaded from one JSON artifact
/// with validate-on-decode semantics on every field.
///
/// Unknown fields are rejected so that a params file written for a newer
/// node version (e.g. one carrying fork activations this binary does not
...
impl AlpenParams {
    ...
    /// Returns the Alpen fork schedule.
    pub fn alpen_forks(&self) -> &AlpenForkSchedule {
        &self.alpen_forks
    }
