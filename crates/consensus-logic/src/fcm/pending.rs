...
impl PendingBlockCache {
        ...
        self.entries
            ...
            .filter(|(block, entry)| {
                (entry.failures > 0
                    ...
                    && (entry.reason != ExecutionDeferral::Storage || entry.next_retry <= now)
                    && (entry.action == PendingAction::Cleanup
    ...
    /// Storage-deferred entries retain their retry deadlines even when the cache is full.
    /// Ready entries keep their places until attempted; only blocked entries age out.
    ...
    /// Storage-deferred entries remain protected until their retry deadline.
    ...
    /// Delays dependency retries by one second and storage retries by exponential backoff
    /// capped at 32 seconds. Long-lived entries emit a warning at most every five minutes.
    ...
    ///
    /// Progress bypasses dependency delays, but preserves storage backoff. Non-genesis
    ...
    /// the batch in slot order. Entries remain cached until resolved or evicted.
...
mod tests {
    ...
    fn missing_parent_outside_queue_blocks_child_until_parent_attaches() {
        ...
        assert!(pending.select_retry_batch(true, 32, |_| false).is_empty());
        assert_eq!(
    ...
    fn aged_ready_entries_keep_their_first_attempt() {
        ...
        assert_eq!(pending.select_retry_batch(true, 1, |_| true), vec![block]);
    ...
    fn storage_backoff_is_capped_and_progress_does_not_bypass_it() {
        ...
        for _ in 0..40 {
            pending.record_storage_failure(OLBlockCommitment::new(1, id));
        }
        ...
        assert!(pending.select_retry_batch(true, 32, |_| true).is_empty());
        pending
            .entries
