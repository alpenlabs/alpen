...
impl<C: FcmContext> FcmServiceState<C> {
    ...
    pub(super) fn is_execution_ready(&self, block: &OLBlockV1) -> bool {
        self.pending.is_parent_ready(
            block.header().slot(),
            block.header().parent_blkid(),
            &|parent| self.inner_state.chain_tracker.is_seen_block(parent),
        )
    }
    ...
    pub(super) fn select_pending_retry_batch(
        ...
        self.pending
            .select_retry_batch(progress, limit, attempted, |parent| {
...
pub(crate) async fn init_fcm_service_state<C: FcmContext>(
    ...
    let history_base = fcm_ctx.get_history_base().await?;
    let rejected_indexing_blocks = collect_rejected_indexing_blocks_for_target(
        cur_tip_block,
        tip_epoch,
        history_base,
        fcm_ctx.as_ref(),
...
async fn collect_rejected_indexing_blocks_for_target(
    ...
    let target_epoch_block_commitments = collect_target_epoch_block_commitments_from_tip(
        target_block,
        target_epoch,
        history_base,
        storage,
...
///
/// The walk stops at the history base because promoted checkpoint-sync
/// datadirs may retain only the anchor header/state, not older intra-epoch
/// parents.
async fn collect_target_epoch_block_commitments_from_tip(
    ...
    let history_base_block = history_base.map(|base| base.to_block_commitment());
    ...
    loop {
        ...
        if Some(commitment) == history_base_block || header.slot() == 0 {
            break;
        }
