...
use strata_identifiers::{
    Buf32, Epoch, EpochCommitment, L1BlockCommitment, L1BlockId, L1Height, OLBlockCommitment,
    OLBlockId, SubjectId, WtxidsRoot,
};
    ...
    for forged in variants {
        assert!(matches!(
            ...
            Err(WorkerError::StfExecution(
                ExecError::AsmManifestContentMismatch { height: 2 }
            ))
        ));
    }
}
