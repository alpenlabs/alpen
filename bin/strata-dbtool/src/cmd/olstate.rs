...
use strata_identifiers::{Buf32, Epoch, OLBlockCommitment, OLBlockId, Slot};
use strata_ledger_types::IStateAccessor;

use crate::{
    cli::OutputFormat,
    output::{olstate::OLStateInfo, output},
...
pub(crate) fn get_ol_state(
    ...
    // OL state currently exposes ASM-recorded epoch directly. Use it for both fields.
    ...
    let ol_state_info = OLStateInfo {
        ...
        l1_next_expected_height: u64::from(top_level_state.last_l1_height()) + 1,
        l1_safe_block_height: u64::from(top_level_state.last_l1_height()),
...
pub(crate) fn revert_ol_state(
    ...
    let finalized_slot = tip_state.asm_recorded_epoch().last_slot();
...
fn parse_ol_block_id(input: &str) -> Result<OLBlockId, DisplayedError> {
    Ok(OLBlockId::from(Buf32::from(parse_block_id_hex(input)?)))
}
