    ...
    executor.spawn_critical_async("checkpoint-proof-runner", async move {
        ...
        loop {
            ...
            while next_epoch_to_prove <= latest_epoch {
                ...
                let commitment = match storage.find_valid_epoch_commitment_at_blocking(epoch) {
                    Ok(Some(c)) => c,
                    Ok(None) => {
                ...
                match prover_handle.execute(task).await {
                    Ok(TaskResult::Completed { task: _ }) => {
                        ...
                        let latest_commitment = match storage.find_valid_epoch_commitment_at_blocking(epoch) {
                            Ok(Some(c)) => c,
                            Ok(None) => {
                                warn!(
                                    %epoch,
