//! Handler for `strataee_getBlockStatus`.
...
use alpen_ee_rpc_api::{AlpenClientRpcServer, BlockStatus, BlockStatusResponse};
use async_trait::async_trait;
use jsonrpsee::core::RpcResult;
use strata_acct_types::Hash;
use tokio::sync::watch;

use super::errors::{block_not_found_error, frontier_unavailable_error, internal_error};

/// RPC handler for [`AlpenClientRpcServer`].
pub(crate) struct BlockStatusRpc<S> {
    storage: Arc<S>,
...
impl<S> BlockStatusRpc<S> {
    pub(crate) fn new(storage: Arc<S>, consensus_rx: watch::Receiver<ConsensusHeads>) -> Self {
        Self {
            storage,
            consensus_rx,
        }
...
impl<S: BatchStorage + ExecBlockStorage> BlockStatusRpc<S> {
...
#[async_trait]
impl<S> AlpenClientRpcServer for BlockStatusRpc<S>
where
