...
//! - Output 1: P2TR with the operator-aggregated internal key and a takeback tapscript (recovery_pk
//!   + relative timelock).
//!
...
pub struct ComputeDrtOutputArgs {
    ...
    /// destination account serial. Serials `0..SYSTEM_RESERVED_ACCTS`
    /// (that is, `0..=127`) are reserved for system slots; `128` is the
    /// first non-reserved serial and is where the Alpen EE account sits.
    /// Default targets the Alpen EE account.
...
pub(crate) fn compute_drt_output(args: ComputeDrtOutputArgs) -> Result<(), DisplayedError> {
    ...
    let subject = SubjectIdBytes::try_new(alpen_addr_bytes).ok_or_else(|| {
        DisplayedError::UserError("alpen_address subject too long".to_string(), Box::new(()))
    })?;
...
fn parse_network(s: &str) -> Result<Network, DisplayedError> {
    // Note: legacy `Network::Testnet` (testnet3) is being sunsetted, so we
    // map the bare `testnet` string to `Testnet4`. If someone still needs
    // testnet3 they can add an explicit `testnet3` arm.
    match s {
