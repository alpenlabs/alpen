...
/// Arguments for creating a deposit transaction.
///
...
pub(crate) fn create_deposit_tx(args: CreateDepositTxArgs) -> Result<(), DisplayedError> {
    let tx_bytes = hex::decode(&args.drt_tx).user_error("Invalid DRT hex format")?;
    ...
    let keys_bytes: Result<Vec<[u8; 78]>, DisplayedError> = keys
        .iter()
        .map(|k| {
            let bytes = hex::decode(k).user_error("Invalid operator key hex format")?;
