...
pub(crate) fn sign_schnorr_sig_inner(
    ...
    let message = Buf32::from_str(message).map_err(|_| Error::TxBuilder("invalid message hash".to_string()))?;
    let sk = Buf32::from_str(secret_key).map_err(|_| Error::TxBuilder("invalid secret key".to_string()))?;

    let sig = sign_schnorr_sig_crypto(&message, &sk);

    // get the public key
    let sk = SecretKey::from_str(secret_key).map_err(|_| Error::TxBuilder("Invalid secret key".to_string()))?;
