...
def load_slots_per_epoch(datadir: str) -> int:
    """Load `epoch_sealing.slots_per_epoch` from config.toml and fail if missing/invalid."""
    config_path = Path(datadir) / "config.toml"
    if not config_path.exists():
        raise AssertionError(f"Missing config file: {config_path}")

    try:
        with open(config_path) as f:
            config = toml.load(f)
    except Exception as exc:
        raise AssertionError(f"Failed to parse config file {config_path}: {exc}") from exc

    slots = config.get("epoch_sealing", {}).get("slots_per_epoch")
    if not isinstance(slots, int) or slots <= 0:
        raise AssertionError(
            f"Invalid or missing epoch_sealing.slots_per_epoch in {config_path}: {slots!r}"
        )
...
def _target_start_of_epoch_from_tip(
    datadir: str, tip_block_id: str, prev_terminal_slot: int
) -> tuple[str, int]:
    """
    ...
    slots_per_epoch = load_slots_per_epoch(datadir)
...
def target_start_of_checkpointed_epoch(
    ...
    return _target_start_of_epoch_from_tip(datadir, checkpoint["tip_l2_blkid"], prev_terminal_slot)
...
def setup_revert_ol_state_test(
    ...
    *,
    ...
    """Set up sequencer + bitcoin state for OL state revert tests."""
    ...
    start_slot = strata_service.get_cur_block_height(strata_rpc)
    ...
    sync_status = strata_service.get_sync_status(strata_rpc)
    ...
    return {
        ...
        "datadir": strata_service.props["datadir"],
        "start_slot": start_slot,
        "tip_slot": sync_status["latest"]["slot"],
        "tip_blkid": sync_status["latest"]["blkid"],
    }
...
def setup_revert_ol_state_test_fullnode(
    ...
    *,
    ...
    """Set up sequencer + fullnode + bitcoin state for OL state revert tests."""
    seq_ctx = setup_revert_ol_state_test(
        seq_service,
        btc_service,
        target_epoch=target_epoch,
    ...
    fn_service.wait_for_block_height(
        seq_ctx["start_slot"] + fn_min_sync_delta, rpc=fn_rpc, timeout=fn_sync_timeout
    )
    fn_tip_slot = fn_service.get_sync_status(fn_rpc)["latest"]["slot"]

    return {
        "seq_service": seq_service,
        "fn_service": fn_service,
        "btc_service": btc_service,
        "seq_rpc": seq_ctx["rpc"],
        "fn_rpc": fn_rpc,
        "btc_rpc": seq_ctx["btc_rpc"],
        "mine_address": seq_ctx["mine_address"],
        "seq_datadir": seq_service.props["datadir"],
        "fn_datadir": fn_service.props["datadir"],
        "start_slot": seq_ctx["start_slot"],
        "seq_tip_slot": seq_ctx["tip_slot"],
        "seq_tip_blkid": seq_ctx["tip_blkid"],
        "fn_tip_slot": fn_tip_slot,
    }


def get_latest_checkpoint(datadir: str) -> dict[str, Any]:
    """Fetch latest checkpoint, epoch summary, and related metadata."""
    ...
    # Prefer the summary's expected count (mapped from last checkpoint epoch in dbtool),
    # then scan down until we find a checkpoint that actually exists.
    start_epoch = int(expected_checkpoints) if isinstance(expected_checkpoints, int) else 0
    start_epoch = max(start_epoch, checkpoints_found - 1, 0)
    latest_epoch = None
    ...
    if latest_epoch is None:
        raise AssertionError(
            "No checkpoint entry found while scanning summary-derived epochs "
            f"(start_epoch={start_epoch}, checkpoints_found_in_db={checkpoints_found})"
        )

    checkpoint = run_dbtool_json(datadir, "get-checkpoint", str(latest_epoch))
    epoch_summary = run_dbtool_json(datadir, "get-epoch-summary", str(latest_epoch))
    return {
        "datadir": datadir,
...
def target_end_of_checkpointed_epoch(checkpoint: dict[str, Any]) -> tuple[str, int]:
    ...
    return checkpoint["tip_l2_blkid"], int(checkpoint["tip_l2_slot"])
